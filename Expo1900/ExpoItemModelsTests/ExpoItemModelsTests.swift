...
//MARK: - JSON Modeling Test
    func test_Json데이터를_ExpoItemElement타입의배열로변환후_그중하나의인스턴스로_정상변환되었는지확인_() throws {
        ...
        let decodedData = try jsonDecoder.decode([ExpoItemElement].self, from: data)
        for item in decodedData where item.name == "직지심체요절" {
            expoItem = item
            break
        }
        
        //then
        XCTAssertEqual(result.name, expoItem.name)
        XCTAssertEqual(result.imageName, expoItem.imageName)
        XCTAssertEqual(result.shortDesc, expoItem.shortDesc)
        XCTAssertEqual(result.desc, expoItem.desc)
    }
}
