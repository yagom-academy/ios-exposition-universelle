...
final class EntryTests: XCTestCase {
    ...
    func test_sut첫번째의_image은_json의image_name과같다() {
        // given
        let expectation = "jikji"
        // when
        let result = sut.first?.image
        // then
        XCTAssertEqual(expectation, result)
    }
    
    func test_sut첫번째의_shortDescription은_json의short_desc과같다() {
        // given
        let expectation = "백운화상 경한(景閑)이 1372년에 초록한 불교 서적"
        // when
        let result = sut.first?.shortDescription
        // then
        XCTAssertEqual(expectation, result)
    }
    
    func test_sut첫번째의_description은_json의desc와같다() {
        // given
        let expectation = """
...
"""
        // when
        let result = sut.first?.description
        // then
        XCTAssertEqual(expectation, result)
    }
}
