...
""")
        XCTAssertEqual(result, expos.first)
    }
}

extension WorldFairPoster: Equatable {
    public static func ==(lhs: WorldFairPoster, rhs: WorldFairPoster) -> Bool {
        return lhs.title == rhs.title && lhs.visitors == rhs.visitors && lhs.description == rhs.description && lhs.duration == rhs.duration && lhs.location == rhs.location
    }
}
