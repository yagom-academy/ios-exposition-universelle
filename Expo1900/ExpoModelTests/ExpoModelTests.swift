...
final class ExpoModelTests: XCTestCase {
    func test_파리만국박람회json데이터를_parsing했을때_정보를추출한다() {
		...
		let visitorExpectation: UInt = 48130300
        ...
        // then
        XCTAssertEqual(expo.title, titleExpectation)
        XCTAssertEqual(expo.visitors, visitorExpectation)
        XCTAssertEqual(expo.location, locationExpectation)
        XCTAssertEqual(expo.duration, durationExpectation)
        XCTAssertEqual(expo.description, descriptionExpectation)
    }
}
